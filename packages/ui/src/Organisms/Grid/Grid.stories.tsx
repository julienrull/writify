...
export const Primary = {
        render: (args: any) => {
                        ...
                        if(nodeToDeleteDependencies){
                                ...
                                if(nodeParentDependencies){
                                        if(nodeParent.type !== "ROOT") {
                                                let nodeSibling = nodeParentDependencies.filter(node => node.type !== 'ROOT' && node !== nodeToDelete)[0];
                                                let nodeParentOfParent = nodeParentDependencies[0];
                                                let nodeParentOfParentDependencies = gridState.get(nodeParentOfParent);
                                                if(nodeParentOfParentDependencies) {
                                                        ...
                                                        if(nodeSiblingDependencies) {
                                                                ...
                                                                nodeSiblingDependencies = nodeSiblingDependencies.filter(node => node !== nodeParent)
                                                ...
                                                nodeParentDependencies = nodeParentDependencies.filter(node => node !== nodeToDelete);
                ...
                return<Grid {...args} removeNode={removeNode} />
        },
        args:{
                grid: gridState,
        }
